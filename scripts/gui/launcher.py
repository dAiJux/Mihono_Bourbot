...
import ctypes
from .vision_test import VisionTestDialog
from .click_test import ClickTestDialog
...
class BotLauncher(tk.Tk):
    ...
    def _build_auto_tab(self, parent):
        ...
        ttk.Button(
            tgrp, text="Click Test", command=self._test_click
        ).pack(side="left", padx=8, pady=8)
    ...
    def _test_click(self):
        self._save_config()
        self._log("Opening Click Test...")
        try:
            from scripts.gui.prereqs import bootstrap_libs, preload_cv2
            bootstrap_libs()
            preload_cv2()
            ClickTestDialog(self, CONFIG_PATH)
        except Exception as e:
            full_tb = traceback.format_exc()
            self._log("ERROR: " + str(e))
            for line in full_tb.splitlines():
